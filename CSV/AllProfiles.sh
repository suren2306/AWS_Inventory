...
SkipProfiles=()
SkipProfiles=("Org-Test1" "Org-Test2" "Nope" "Personal" "#Nope" "#Personal" "SS-default" "SS-seciaas" "SS-Sand" "TouchPoint" "Shared-Fid" "TIAA-Mngmnt-Prod-e1" "TIAA-Mngmnt-Prod-w2" "ChildAccount-e1" "ChildAccount-w2" "ChildAccount2-e1" "ChildAccount2-w2")
...
if [[ " ${automated} " =~ " AllProfiles " ]]; then # If user wants to see ALL profiles
...
for profile in ${CredProfiles[@]}; do
	if [[ ! " ${SkipProfiles[@]} " =~ " ${profile} " ]]; then
		if [[ (" ${automated} " =~ " ProfileNameOnly ") || (" ${automated} " =~ " AllProfiles ") ]]; then
