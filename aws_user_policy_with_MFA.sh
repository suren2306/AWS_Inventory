...
aws iam list-attached-user-policies --user-name welkin --profile default --output text --query 'AttachedPolicies[].PolicyName' | tr '\t' '\n' | awk -F $"\t" -v var=default -v var2=welkin '{printf "%-15s %-40s %-60s \n",var,var2,$1}'